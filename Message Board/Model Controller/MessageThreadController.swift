...
class MessageThreadController {
    ...
    func fetchMessageThreads(completion: @escaping () -> Void) {
        ...
        URLSession.shared.dataTask(with: requestURL) { (data, _, error) in
            ...
            do {
                let results = try JSONDecoder().decode([String: MessageThread].self, from: data).values
                self.messageThreads = Array(results)
            } catch {
                self.messageThreads = []
    ...
    static let baseURL = URL(string: "https://message-board-96b2b.firebaseio.com/")!
