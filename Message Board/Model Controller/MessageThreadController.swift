...
class MessageThreadController {
    ...
    func fetchMessageThreads(completion: @escaping () -> Void) {
        ...
        URLSession.shared.dataTask(with: requestURL) { (data, _, error) in
            ...
            do {
                ...
                self.messageThreads = messageThreadsDictionary.map({ $0.value })
    ...
    func createMessage(in messageThread: MessageThread, withMessageText messageText: String, sender: String, completion: @escaping () -> Void) {
        
        // This if statement and the code inside it is used for UI Testing. Disregard this when debugging.
        if isUITesting {
            createLocalMessage(in: messageThread, withText: messageText, sender: sender, completion: completion)
            return
        }
